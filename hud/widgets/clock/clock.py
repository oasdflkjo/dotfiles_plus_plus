...
class ClockWidget(BaseWidget):
    ...
    def update_time(self):
        ...
        time_format = (
            "%H:%M:%S" if self.config["show_seconds"] else self.config["time_format"]
        )
...
class ClockCanvas(QWidget):
    ...
    def paintEvent(self, event):
        ...
        # Calculate max width for group centering
        max_width = max(time_rect.width(), date_rect.width())
        box_left = (self.width() - max_width) // 2

        # Center each line within the box
        time_x = box_left + (max_width - time_rect.width()) // 2 - time_rect.x()
        date_x = box_left + (max_width - date_rect.width()) // 2 - date_rect.x()
